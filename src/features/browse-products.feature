Feature: Browse products
  ...
  Scenario Outline: UC-10 Sidebar | Filters: Select independent filters for <filter_group>
    ...
    Then the "<value>" checkbox should be marked as selected
      And the product grid should filter and display items matching "<value>"
      And the search results header should reflect the applied filter
  ...
  Scenario Outline: UC-11 Sidebar | Category: Sync selection between parent and subcategories
    Given the customer is on the products page
    When the customer selects the main category "<parent>"
    Then the category "<parent>" checkbox should be marked as selected
      And all subcategories under "<parent>" should be automatically selected
      And the product grid should display all items belonging to "<parent>"
